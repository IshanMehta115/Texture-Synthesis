...
public:
    ...
    vector<vector<double>> get_analogy(CYLINDER *c)
    {
        ...
        for(int i=0;i<points_curve.size();i+=3)
        {
            ...
            double R1 = sqrt((points_curve[i] * points_curve[i]) + (points_curve[i + 1] * points_curve[i + 1]) + (points_curve[i + 2] * points_curve[i + 2]));
            double R2 = sqrt(((points_of_c[i] - 1000.0) * (points_of_c[i]-1000.0)) + (points_of_c[i + 1] * points_of_c[i + 1]) + (points_of_c[i + 2] * points_of_c[i + 2]));
            // cout<<"dx = "<<dx<<"\n";
            // cout<<"dy = "<<dy<<"\n";
            // cout<<"dz = "<<dz<<"\n\n";
...
void createCubeObject(unsigned int &program, unsigned int &cube_VAO)
{
    ...
    cylinder_2->set_offset(1000);
    cylinder_2->set_radius(18);
    cylinder_2->set_height(40);
    cylinder_2->fill_points(1);



    vector<vector<double>> input_analogy = cylinder_1->get_analogy(cylinder_2);
