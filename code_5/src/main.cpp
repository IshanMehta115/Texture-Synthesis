...
private:
    ...
    double func(int mode,int i,int j)
    {
        ...
        i-=n/2;
        j-=n/2;
        double val = cos(i/5.0)*cos(j/5.0);
        val+=1;
        val/=2;
        val = 1-val;
        val = pow(val,100);
...
public:
    ...
    void fill_points(int mode)
    {
        ...
        {
            points_caps.push_back(x_offset);
            points_caps.push_back(-height/2);
            points_caps.push_back(0);

            points_caps.push_back(x_offset + Radius * cos(get_theta_angle((theta_step + 1)%total_theta_steps)));
            points_caps.push_back(-height/2);
            points_caps.push_back(Radius * sin(get_theta_angle((theta_step + 1)%total_theta_steps)));

            points_caps.push_back(x_offset + Radius * cos(get_theta_angle((theta_step)%total_theta_steps)));
            points_caps.push_back(-height/2);
            points_caps.push_back(Radius * sin(get_theta_angle((theta_step)%total_theta_steps)));
