...
private:
    ...
    void sphere_parametric_equation(int mode, float phi, float theta, double *coordinates)
    {
        ...
        if(1==mode)
        {
            R+=pow((cos(theta * 6) * cos(phi * 4)),20) * 4;
        }
        else if(2==mode)
        {
            float u = phi/PI;
...
public:
    ...
    vector<vector<double>> get_analogy(SPHERE *s)
    {
        vector<vector<double>> analogy(n, vector<double>(n));
        vector<double> points_of_s = s->get_points();
        for(int i=0;i<points.size();i+=3)
        {
            double theta = atan2(points[i + 1], points[i]);
            double phi = acos(points[i + 2]/Radius);
            ...
            ii%=101;
            jj%=101;
            double R1 = sqrt((points[i] * points[i]) + (points[i + 1] * points[i + 1]) + (points[i + 2] * points[i + 2]));
            double R2 = sqrt(((points_of_s[i] - 1000.0) * (points_of_s[i]-1000.0)) + (points_of_s[i + 1] * points_of_s[i + 1]) + (points_of_s[i + 2] * points_of_s[i + 2]));
            double val = R2-R1;
            analogy[ii][jj] = val;
        }
        return analogy;
...
public:
    ...
    double X(int mode, double theta,double z)
    {
        ...
        if(1==mode)
        {
            R+=pow((cos(theta * 6) * cos(z/3)),20) * 3;  
        }
        else if(2==mode)
        {
            float u = ((z * ((total_z_steps - 1)/height)) + 15) / (total_z_steps);
    ...
    vector<vector<double>> get_analogy(CYLINDER *c)
    {
        vector<vector<double>> analogy(n, vector<double>(n));
        vector<double> points_of_c = c->get_points_curve();
        for(int i=0;i<points_curve.size();i+=3)
        {
            double z = points_curve[i+1];
            double theta = atan2(points_curve[i + 2], points_curve[i]) + PI;
            ...
            ii%=101;
            jj%=101;
            double dx = points_curve[i] - (points_of_c[i] -1000.0);
            double dy = points_curve[i + 1] - points_of_c[i + 1];
            double dz = points_curve[i + 2] - points_of_c[i + 2];
...
void createCubeObject(unsigned int &program, unsigned int &cube_VAO)
{
    ...
    sphere_1->set_offset(0);
    sphere_1->set_radius(30);
    sphere_1->fill_points(0);


    SPHERE *sphere_2 = new SPHERE();
    sphere_2->set_offset(1000);
    sphere_2->set_radius(30);
    sphere_2->fill_points(1);


    vector<vector<double>> input_analogy = sphere_1->get_analogy(sphere_2);
